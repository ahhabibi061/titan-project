...
// -------------------- DOMAIN --------------------
const GOALS = [
  { id: 'cut',      label: 'Cut',      desc: 'Lose body fat while preserving muscle',     offset: -300 },
  { id: 'bulk',     label: 'Bulk',     desc: 'Build muscle with controlled fat gain',      offset: +300 },
  { id: 'recomp',   label: 'Recomp',   desc: 'Slow body recomposition at maintenance',     offset: 0 },
  { id: 'maintain', label: 'Maintain', desc: 'Hold current weight, focus on performance',  offset: 0 },
];
...
const ACTIVITY_LEVELS = [
  ...
  { id: 'active',      label: 'Active',       desc: 'Athlete or 6+ sessions / week',               mult: 1.725 },
  { id: 'very_active', label: 'Very Active',  desc: 'Hard daily training or physical job + gym',   mult: 1.9 },
];
...
function calcMacros({ weightKg, heightCm, age, sex, activity, goal }) {
  ...
  const tdee = bmr * mult;
  const kcalOffset = GOALS.find(g => g.id === goal)?.offset ?? 0;
  const kcal = Math.round(tdee + kcalOffset);

  // Protein: 1g per lb of bodyweight (weight_kg × 2.205)
  const protein = Math.round(weightKg * 2.205);

  // Fat: 25% of total kcal ÷ 9
  ...
  return (
    <div>
      ...
      <input
        ...
        onChange={(e) => onChange(e.target.value)}
        placeholder={placeholder}
        className="w-full bg-stone-950/60 border border-stone-800 px-4 py-3 text-stone-100 font-anton text-2xl focus:outline-none focus:border-orange-500/60"
      />
    </div>
  );
}
...
export default function Onboarding() {
  ...
  const canAdvance = useMemo(() => {
    switch (step) {
      case 0: return !!goal;
      case 1: return displayName.trim() && weight && height && age && sex;
      case 2: return goalWeight && timeline;
      case 3: return !!program && !!experience;
      default: return true;
    }
  }, [step, goal, displayName, weight, height, age, sex, goalWeight, timeline, program, experience]);
  ...
  const handleSubmit = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        start_weight_kg:    parseFloat(weight),
        ...
        goal,
        current_macros:     { kcal: macros.kcal, protein: macros.protein, carbs: macros.carbs, fat: macros.fat },
        ...
        <div key={step} className="reveal">
          ...
          {step === 4 && (
            <div>
              ...
              {/* SAVE ERROR */}
              {saveError && (
                <div className="mb-4 px-4 py-3 border border-red-500/30 bg-red-500/10 text-red-300 font-mono text-xs">
                  {saveError}
                </div>
              )}
            </div>
        ...
        <div className="mt-10 pt-6 border-t border-stone-800/60 flex items-center justify-between gap-3">
          ...
          {step < STEPS.length - 1 ? (
            ...
            <button
              ...
              disabled={saving}
              className="px-7 py-2.5 bg-orange-500 text-stone-950 font-anton text-base uppercase tracking-wider hover:bg-orange-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
